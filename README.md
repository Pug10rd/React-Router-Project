...
const [search, setSearch] = useState(''); const [result, setResult] =
useState([]);

useEffect(() => { const options = { method: 'GET', headers: { accept:
'application/json', Authorization: 'Bearer
eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI1ZjEyY2UyZjY2MjRkYTlhZWJlOWI1MWRmMmI3MTZhMCIsInN1YiI6IjY0MmVjNmEzMGQyZjUzMDA5NzM3ZDNhMyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.3y5LPiD5o4iA1k2X5yCEkyqG7cW3QxDYX5JfmTs7Ytw',
}, };
...
return ( <> <div>1</div> <div>2</div> <input className="searchbar"
placeholder="Search..." onChange={e => setSearch(e.target.value)} /> <ul>
{result.map(item => { return <li key={item.id}>{item.title}</li>; })} </ul> </>
);
